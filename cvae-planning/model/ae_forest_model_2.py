...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size=32, output_size=32):
            ...
            self.encoder = nn.Sequential(
                nn.Conv2d(in_channels=3, out_channels=16, kernel_size=[5,5], stride=[2,2]),  # 99
                nn.ReLU(),
                nn.MaxPool2d(2, stride=2),                                                  # ~50
                nn.Conv2d(in_channels=16, out_channels=32, kernel_size=[5,5], stride=[2,2]), # ~22
                nn.ReLU(),
                nn.MaxPool2d(2, stride=2),                                                  # ~11
                nn.Conv2d(in_channels=32, out_channels=64, kernel_size=[5,5], stride=[2,2]), # ~4x4
                nn.ReLU()
        )
